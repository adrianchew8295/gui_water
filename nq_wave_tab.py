# 文件名: nq_wave_tab.py
# 核心功能: 專業 TradingView 波浪可視化 (支援 Fib 價格線與時間週期線獨立開關 / AI 大白話分析 Prompt)
...
def render_tradingview_wave_chart(df_day: pd.DataFrame, wave_res: dict, show_fib_price: bool = True, show_fib_time: bool = True):
    if df_day.empty:
        st.warning("⏳ 暫無有效日線數據。")
        return
    ...
    time_windows_json = json.dumps(wave_res.get('time_window_dates', []))

    show_fib_price_js = "true" if show_fib_price else "false"
    show_fib_time_js = "true" if show_fib_time else "false"
    ...
    <head>
        ...
        <style>
            ...
            .legend-bar {{
                ...
                background: rgba(13, 17, 23, 0.88); padding: 5px 12px; border-radius: 4px;
    ...
    <body>
        <div class="legend-bar">
            <span style="color:#58a6ff; font-weight:bold;">QQQ/NQ 日線波浪圖</span>
            <span style="color:#ffd700; margin-left:10px;">── 黃線: 骨架軌跡</span>
            {"<span style='color:#00E676; margin-left:10px;'>── Fib 價格防線 [開]</span>" if show_fib_price else "<span style='color:#8b949e; margin-left:10px;'>[Fib 價格關]</span>"}
            {"<span style='color:#a371f7; margin-left:10px;'>│ Fib 時間窗口 [開]</span>" if show_fib_time else "<span style='color:#8b949e; margin-left:10px;'>[時間窗口關]</span>"}
        </div>
        ...
        <script>
            function initChart() {{
                ...
                const chart = LightweightCharts.createChart(chartContainer, {{
                    width: chartContainer.clientWidth || window.innerWidth || 800,
                    height: 500,
                    layout: {{ background: {{ color: '#0d1117' }}, textColor: '#8b949e', fontSize: 12 }},
                ...
                // 1. 斐波那契價格回調線
                const showFibPrice = {show_fib_price_js};
                const fibLevels = {fib_levels_json};
                if (showFibPrice && Object.keys(fibLevels).length > 0) {{
                    const colors = {{
                        "0.236": "#ff7b72",
                        "0.382 (4浪常規支撐)": "#d29922",
                        "0.500 (平衡防線)": "#58a6ff",
...
def render_nq_wave_prediction_dashboard():
    ...
    st.caption("依據文獻: **Robert R. Prechter & A.J. Frost《Elliott Wave Principle》原著算法**")

    df_day = load_data("US.QQQ", "DAY")
    df_1h = load_data("US.QQQ", "1Hr")

    if df_day.empty:
        st.warning("⏳ 尚未檢測到 `US_QQQ_DAY.csv` 歷史數據，請先運行 `python data_fetcher.py`！")
        return
    ...
    m3.metric("🧬 浪級結構屬性", wave_res["complex_type"])
    m4.metric("⏱️ 運行時間跨度", f"{wave_res['time_elapsed_bars']} 棒/天", f"預期週期 ~{wave_res['expected_duration_bars']} 棒")

    st.markdown("---")

    # 2. 控制列：Fibonacci 價格線與時間週期獨立開關
    c_title, c_sw1, c_sw2 = st.columns([2.5, 1.2, 1.3])
    with c_title:
        st.markdown("#### 📈 TradingView 原生日線波浪圖")
    with c_sw1:
        show_fib_p = st.toggle("📐 斐波那契價格線", value=True)
    with c_sw2:
        show_fib_t = st.toggle("⏱️ 費氏時間週期", value=True)

    render_tradingview_wave_chart(df_day, wave_res, show_fib_price=show_fib_p, show_fib_time=show_fib_t)

    st.markdown("---")

    # 3. 斐波那契點位、時間預測與空間推演
    st.markdown("#### 🧭 空間目標推演與費氏時間窗口 (Time & Price Projection)")
    t1, t2, t3 = st.columns(3)
    
    with t1:
        st.markdown("**📐 Fibonacci 價格回調防線**")
    ...
    with t2:
        st.markdown("**🎯 空間目標與風控防禦 (Frost & Prechter)**")
        st.markdown(f"""
        | 指標項目 | 點位 ($) | 算法依據 |
        | :--- | :--- | :--- |
    ...
    with t3:
        st.markdown("**⏱️ 費氏時間序列預測窗口 (Time Windows)**")
    ...
    # 4. 專屬 AI 智能分析 Prompt (要求大白話 + 華爾街新聞)
...
【1. 艾略特波浪量化數據（依據 Frost & Prechter 原著）】
...
• 子浪進展: {wave_res['sub_wave']} (已運行 {wave_res['time_elapsed_bars']} 根日線 Bar)
• 斐波那契關鍵回調位:
...
• 目標預測點位: Target 1 (對稱浪) = ${wave_res['next_target_1']:,.2f} | Target 2 (擴展浪) = ${wave_res['next_target_2']:,.2f}
• 結構失效止損線 (SL): ${wave_res['invalid_price']:,.2f}

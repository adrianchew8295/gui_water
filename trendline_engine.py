...
def find_td_pivots(df: pd.DataFrame, window: int = 4) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for i in range(window, n - window):
        if np.all(highs[i] >= highs[i - window : i]) and np.all(highs[i] >= highs[i + 1 : i + window + 1]):
            td_highs.append({"bar_idx": i, "time": str(times[i]), "price": float(highs[i])})
        if np.all(lows[i] <= lows[i - window : i]) and np.all(lows[i] <= lows[i + 1 : i + window + 1]):
            td_lows.append({"bar_idx": i, "time": str(times[i]), "price": float(lows[i])})
...
def compute_demark_trendlines(df: pd.DataFrame, window: int = 4) -> Dict[str, Any]:
    ...
    # 德馬克目標投影預測 (TD Target Projections)

# 文件名: chart_plugin.py
# 核心特性: Audit Logs 全面注入 K線形態柱體標籤 + 雙表對齊 + 1H EMA20 門禁 + 5M 2B 當根定罪
...
class ChartPlugin:
    ...
    def render_cockpit(self, code: str, budget_usd: float = 200.0):
        ...
        for idx, row in bars_6.iterrows():
            ...
            # 形態分類解剖
            shape_str = StrategyEngine.classify_candle_shape(o, h, l, c)
            ...
            if idx == 0:
                col1_t1 = f"<b style='color:#58a6ff;'>{t_str}</b> <span style='background:#1f293d; color:#58a6ff; padding:2px 6px; border-radius:4px; font-weight:bold;'>⚡ LIVE</span> <span style='color:#8b949e; font-size:11px;'>⏱️ {countdown_str}</span>"
                col1_t2 = f"<b style='color:#58a6ff;'>{t_str}</b> <span style='color:#8b949e;'>{td_s}</span>"
                c_display = f"<span style='color:{flash_color}; font-weight:bold;'>${curr_price:,.2f} ({flash_sym})</span> [{shape_str}]"
                ...
                # 審核日誌記錄（注入形態標籤）
                audit_log_line = f"• {t_str} ⚡ LIVE  | 現價: ${curr_price:,.2f} ({flash_sym}) [{shape_str}] | 當根極值: {h_display}/{l_display} | 狀態: 即時撮合動態推進"
            else:
                ...
                # 審核日誌記錄（注入形態標籤）
                vol_unit = f"({v:.2f} BTC)" if "BTC" in code.upper() else f"({int(v)} 股)"
                audit_log_line = f"• {t_str} 🔒 收盤  | 收盤: ${c:,.2f} [{shape_str}] | 影線: {h_display}/{l_display} | 量: {vol_ratio:.2f}x {vol_unit} | TD: {td_s} | 診斷: {diag_str}"
        ...
        with col_t1:
            with stylable_container(
                ...
                t1_html += "<tr style='border-bottom: 2px solid #30363d; color: #8b949e;'><th>5M時段與狀態</th><th>現價/收盤 (Close)</th><th>影線極值 (High / Low)</th><th>5M量能 (VPA)</th></tr>"
        ...
        audit_text += f"[5M 閉合時序審核 (過去 6 根真實 K 線)]\n"
